...
class LembagaSearchFragment : Fragment(), PencarianInterfacr {
    private var _binding: FragmentSearchLembagaBinding? = null
    ...
    private lateinit var lembagaKampus: List<LembagaKampus>

    private lateinit var searchLembagaAdapter: SearchLembagaAdapter
    ...
    override fun onCreateView(
        ...
        _binding = FragmentSearchLembagaBinding.inflate(inflater, container, false)
    ...
    private fun loadData(query: String) {
        onLoadingSearch()
        ApiClient.instances.getLembagaQuery(query)
            ?.enqueue(object : Callback<Responses.ResponseSearch> {
                override fun onResponse(
                    ...
                    if (response.isSuccessful) {
                        ...
                        if (kode.equals("1")) {
                            lembagaKampus = response.body()?.search_lembaga_data!!
                            if (lembagaKampus.size > 0) {
                                onReadySearch()
                                val rv_loker = binding.rvLembaga
                                val layoutManagerLoker: RecyclerView.LayoutManager =
                                    LinearLayoutManager(activity)
                                rv_loker.layoutManager = layoutManagerLoker
                                searchLembagaAdapter = SearchLembagaAdapter(lembagaKampus)
                                rv_loker.adapter = searchLembagaAdapter
                            } else {
