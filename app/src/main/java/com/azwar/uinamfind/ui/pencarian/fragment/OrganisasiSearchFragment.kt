...
import com.azwar.uinamfind.databinding.FragmentSearchOrganisasiBinding
...
class OrganisasiSearchFragment: Fragment(), PencarianInterfacr {
    private var _binding: FragmentSearchOrganisasiBinding? = null
    ...
    private lateinit var organisasi: List<Organisasi>
    ...
    override fun onCreateView(
        ...
        _binding = FragmentSearchOrganisasiBinding.inflate(inflater, container, false)
    ...
    private fun loadData(query: String) {
        onLoadingSearch()
        ApiClient.instances.getOrganisasiQuery(query)
            ?.enqueue(object : Callback<Responses.ResponseSearch> {
                override fun onResponse(
                    ...
                    if (response.isSuccessful) {
                        ...
                        if (kode.equals("1")) {
                            organisasi = response.body()?.search_organisasi_data!!
                            if (organisasi.size > 0) {
                                onReadySearch()
                                val rv_loker = binding.rvOrganisasi
                                val layoutManagerLoker: RecyclerView.LayoutManager =
                                    LinearLayoutManager(activity)
                                rv_loker.layoutManager = layoutManagerLoker
                                organisasiAdapter = OrganisasiAdapter(organisasi)
                                rv_loker.adapter = organisasiAdapter
                            } else {
